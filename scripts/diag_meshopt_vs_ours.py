"""Meshopt vs ours: meshlet quality + wall-clock time.

For each mesh, runs meshopt_buildMeshlets and our joint_learned generator,
measures generation time (force=True bypasses cache), computes structural
metrics + final STRIDE BPV, and emits a markdown table + CSV.

Columns: #M | V mean | T mean/max | T util% | Strips/M | V repl |
         Aspect med | Bnd% | BPV | gen ms

Usage:
    python scripts/diag_meshopt_vs_ours.py
"""
...
GENERATORS = ["meshopt", "joint_learned"]
GEN_LABEL = {"meshopt": "meshopt", "joint_learned": "ours"}

CSV_PATH = ROOT / "bench_meshopt_vs_ours.csv"
...
def _row_for(mesh_name: str, mesh_path: str, gen: str) -> dict:
    # force=True so gen_time_ms reflects fresh build, not cache load.
    prep = load_or_prepare(
        ...
        clean=True, force=True, verbose=False,
    )
    plans = prep["plans"]
    vn = prep["vn"]
    n_v = int(prep["n_v"])
    n_meshlets = len(plans)
    gen_time_ms = float(prep.get("gen_time_ms", 0.0))
...
def _print_md_table(rows: list[dict]) -> None:
    cols = [
        ...
        ("gen ms",     lambda r: f"{r['gen_time_ms']:.0f}",            "---:"),
    ]
    print()
    print("| " + " | ".join(name for name, _, _ in cols) + " |")
    print("|" + "|".join((a or "---") for _, _, a in cols) + "|")
    for r in rows:
    ...
    for mesh_name, mesh_path in MESHES:
        for gen in GENERATORS:
            ...
            print(
                ...
                f"v_util={row['v_util_pct']:.0f}%  "
                f"t_util={row['t_util_pct']:.0f}%  "
                f"v_repl={row['vert_replication']:.2f}  "
                f"bpv={row['bpv']:.2f}  "
                f"gen_ms={row['gen_time_ms']:.0f}"
            )
            rows.append(row)
