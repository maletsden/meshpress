...
- **Per-frame GPU coders.** AMD's DGF [1] is the first documented GPU-decode format aimed explicitly at per-frame use, with random access at the granularity of fixed-size (≤ 128-byte) blocks. AMD's published reference reports DGF decode times on a single CPU core (AMD Ryzen 9 7950X), corresponding to approximately 43 M triangles per second across the reported meshes [9]; AMD ships no GPU decoder (§5.1). That single-core CPU figure is not comparable to a GPU decode rate. Our custom re-implementation of the published DGF decode specification, ported to the GPU and instrumented to measure decode alone on a commodity NVIDIA RTX 3090, reaches 1.5–2.2 G triangles per second (§5.3); this is what allows STRIDE and DGF to be compared like-for-like, GPU-to-GPU on identical hardware. The two figures answer different questions and are not directly comparable; the GPU decode-only figure is the relevant comparand for STRIDE's measurements.
...
§5.1 fixes the test corpus, competitor configurations, hardware, and metric definitions. §5.2 reports compression rate against four competitors on eight meshes. §5.3 reports decode throughput against DGF (re-measured on the same hardware) and three CPU competitors. §5.4 decomposes STRIDE's bitstream. §5.5 verifies reconstruction quality and crack-freeness, including the L∞ vertex guarantee.
...
Decode throughput is reported in millions of triangles per second (mesh-independent) against the DGF GPU implementation on the same RTX 3090, together with the three CPU competitors.
...
- **DGF decode-only GPU throughput is much higher than AMD's published CPU decode figure.** AMD GPUOpen reports DGF decode times on a single CPU core (AMD Ryzen 9 7950X) [9]: for instance, 0.15 s for the 7.22 M-triangle Dragon and 0.03 s for the 1.09 M-triangle Happy Buddha, i.e., roughly 36–48 M triangles per second across the reported meshes; AMD ships no GPU decoder. Our re-implementation moves the published decode algorithm onto the GPU (the decoded buffers land in device memory but the rasteriser is not invoked) and on the RTX 3090 reaches 1.5–2.2 G triangles per second. The two figures measure different processors (one CPU, one GPU); only the GPU figure is comparable to the STRIDE numbers in this table, which are likewise GPU decode-only.
...
**Artifacts.** All code, the bitstream specification, raw benchmark CSV files, reproducible scripts, and the test-mesh corpus are publicly available; see §8.
...
- **Dependencies and build.** `requirements.txt` pins the Python environment; the GPU decoder requires CuPy with CUDA 12 and an NVIDIA GPU, and the C++/CUDA benchmark harness (`bench_cpp/`) builds against CUDA 12 with MSVC (Windows) or gcc/clang (Linux). The DGF and Corto baselines are included as git submodules; our custom GPU implementation of the DGF decode specification (§5.1) is part of this harness so that the STRIDE-versus-DGF comparison is GPU-to-GPU on identical hardware.
- **Key algorithms.** The reference encoder (§3.6), the per-mesh integer-rational linear predictor fit (§3.5), the joint-learned meshlet partitioner (§3.2), the strip-emit traversal with packed-local-index connectivity (§3.4–§3.5), and the fused CUDA decoder (§4) each map to a named source file, listed in the repository README.
- **Datasets.** The eight evaluation meshes of Table 2, with their provenance, licenses, and a fetch script: five download automatically, and the three Stanford / Threedscans meshes are retrieved from their original public sources.
- **Reproduction.** `ARTIFACT.md` gives the end-to-end recipe that regenerates the compression-rate, throughput, bit-budget, reconstruction-quality, and VRAM tables together with the paper figures.

This article is the citable reference for STRIDE; its title and journal (*The Visual Computer*) are mirrored in the repository README to aid discovery.
...
35. Szymczak, A.: Optimized Edgebreaker encoding for large and regular triangle meshes. Vis. Comput. 19(4), 271–278 (2003). https://doi.org/10.1007/s00371-003-0208-0

36. Coors, V., Rossignac, J.: Delphi: geometry-based connectivity prediction in triangle mesh compression. Vis. Comput. 20(8–9), 507–520 (2004). https://doi.org/10.1007/s00371-004-0255-1

37. Balreira, D.G., da Silveira, T.L.T.: Triangular matrix-based lossless compression algorithm for 3D mesh connectivity. Vis. Comput. 40(6), 3961–3970 (2024). https://doi.org/10.1007/s00371-024-03400-8
