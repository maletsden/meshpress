...
\item \textbf{Per-frame GPU coders.} AMD's DGF~\cite{ref01_amd_dgf_hpg2024} is the first documented GPU-decode format aimed explicitly at per-frame use, with random access at the granularity of fixed-size ($\le 128$-byte) blocks. AMD's published reference reports DGF decode times on a single CPU core (AMD Ryzen 9 7950X), corresponding to approximately 43~M triangles per second across the reported meshes~\cite{ref09_amd_dgf_blog}; AMD ships no GPU decoder (Section~\ref{sec:setup}). That single-core CPU figure is not comparable to a GPU decode rate. Our custom re-implementation of the published DGF decode specification, ported to the GPU and instrumented to measure decode alone on a commodity NVIDIA RTX 3090, reaches 1.5--2.2~G triangles per second (Section~\ref{sec:throughput}); this is what allows STRIDE and DGF to be compared like-for-like, GPU-to-GPU on identical hardware. The two figures answer different questions and are not directly comparable; the GPU decode-only figure is the relevant comparand for STRIDE's measurements.
\end{itemize}
...
Section~\ref{sec:setup} fixes the test corpus, competitor configurations, hardware, and metric definitions. Section~\ref{sec:rate} reports compression rate against four competitors on eight meshes. Section~\ref{sec:throughput} reports decode throughput against DGF (re-measured on the same hardware) and three CPU competitors. Section~\ref{sec:budget} decomposes STRIDE's bitstream. Section~\ref{sec:recon} verifies reconstruction quality and crack-freeness, including the L$\infty$ vertex guarantee.
...
Table~\ref{tab:budget} decomposes STRIDE's bitstream on three representative meshes from the corpus (small, mid, large). The two residual rows correspond to the two non-anchor vertex kinds of Section~\ref{sec:selfcontained}: \emph{delta} vertices are extra strip-roots within a meshlet (no parallelogram context is available yet) and are coded against the previously decoded vertex, $r_d = q_d - q_d^{(\text{prev})}$; \emph{IRLP} vertices are the interior strip-emit vertices that do have a $(\mathbf{a}, \mathbf{b}, \mathbf{c})$ context and are coded against the integer-rational linear predictor of Section~\ref{sec:irlp}, $r_d = q_d - \hat{q}_d$ with $\hat{q}_d$ from Equation~\eqref{eq:irlp}. Both residual streams are axis-separated and Rice / Exp-Golomb coded under a per-meshlet (tag, $k$) selector. The picture is connectivity-dominated, as expected from the packed-local-index choice (Section~\ref{sec:related} and Section~\ref{sec:irlp}).

\begin{table}[ht]
\caption{Bitstream decomposition (\% of total bytes).}\label{tab:budget}
\begin{tabular*}{\textwidth}{@{\extracolsep\fill}lrrr@{}}
\toprule
Section & Monkey & tank & Dragon \\
\midrule
Per-meshlet headers (n\_local, n\_tris, n\_strips)      & 1.2  & 1.4  & 1.5  \\
...
Side table (off\_bits + counts + axis sub-offs)        & 10.7 & 10.6 & 10.4 \\
\midrule
...
Figure~\ref{fig:budget} extends this per-section accounting across the full eight-mesh corpus.
...
Two properties of STRIDE's design, and not implementation polish, account for the empirical results of Section~\ref{sec:eval}.

\paragraph{A byte saving of $1.38$--$1.93\times$ over DGF on every mesh.}
This arises from two design choices: (i) STRIDE's per-meshlet variable-width fields avoid the fixed 128-byte block alignment that DGF imposes, which is the dominant inflator on small meshlets; (ii) entropy-coded residuals (Rice / Exp-Golomb) sit approximately 20--30\,\% above the per-stream Shannon entropy, whereas DGF's fixed-width per-cluster offset / exponent layout pays a worst-case-fit cost in every cluster. The axis-separated layout of Section~\ref{sec:residuals} adds a 0.4\,\% byte overhead in exchange for an approximately 30\,\% decode-side speedup on the residual phase, a deliberate engineering trade.

\paragraph{Crack-freeness as an invariant of the format.}
Global lattice quantization (Section~\ref{sec:partition}) gives every globally-shared vertex a single integer code, replicated bit-identically in every meshlet that contains it. No per-frame correction, boundary fix-up pass, or application-side deduplication is required. DGF attains the same property through per-cluster offset / exponent metadata that is carefully aligned at meshlet boundaries.

\paragraph{Per-frame off-chip economy.}
The decode-throughput tables in Section~\ref{sec:throughput} measure decode wall-clock only, i.e., the cost a renderer pays once the compressed bytes are already resident on the GPU. They omit a cost that CPU codecs always pay and that STRIDE never pays: the per-frame upload of decoded vertex and index buffers across the host--device link. For scenes containing many small meshes, exactly the regime in which meshoptimizer's CPU decode appears to dominate in Section~\ref{sec:throughput}, the upload bandwidth and submission overhead in fact dominate the apparent decode advantage of the CPU codec. STRIDE keeps the entire scene resident on the GPU in its compressed form.
...
All source code, the STRIDE bitstream specification, the test-mesh corpus, raw benchmark CSV files, and the scripts that regenerate every table and figure in this paper are publicly available in the \textbf{MeshPress} repository at \url{https://github.com/maletsden/meshpress}, released under the MIT license and archived with a permanent DOI on Zenodo (\url{https://doi.org/10.5281/zenodo.20942759}).

The repository documents:

\begin{itemize}
\item \textbf{Dependencies and build.} \texttt{requirements.txt} pins the Python environment; the GPU decoder requires CuPy with CUDA~12 and an NVIDIA GPU, and the C++/CUDA benchmark harness builds against CUDA~12 with MSVC (Windows) or gcc/clang (Linux). The DGF and Corto baselines are included as git submodules; our custom GPU implementation of the DGF decode specification (Section~\ref{sec:setup}) is part of this harness so that the STRIDE-versus-DGF comparison is GPU-to-GPU on identical hardware.
\item \textbf{Key algorithms.} The reference encoder (Section~\ref{sec:residuals}), the per-mesh integer-rational linear predictor fit (Section~\ref{sec:irlp}), the joint-learned meshlet partitioner (Section~\ref{sec:partition}), the strip-emit traversal with packed-local-index connectivity (Sections~\ref{sec:strips}--\ref{sec:irlp}), and the fused CUDA decoder (Section~\ref{sec:decoder}) each map to a named source file, listed in the repository README.
\item \textbf{Datasets.} The eight evaluation meshes of Table~\ref{tab:corpus}, with their provenance, licenses, and a fetch script, documented in a dataset README.
\item \textbf{Reproduction.} \texttt{ARTIFACT.md} gives the end-to-end recipe that regenerates the compression-rate, throughput, bit-budget, reconstruction-quality, and VRAM results together with the paper figures.
\end{itemize}

This article is the citable reference for STRIDE; its title and journal (\emph{The Visual Computer}) are mirrored in the repository README to aid discovery.
...
\item \textbf{Data availability.} All test meshes used in the evaluation are publicly available from the Stanford 3D Scanning Repository, the Blender community asset library, and Threedscans.com (CC0), and are documented with provenance and licenses in a dataset README in the MeshPress repository. The raw benchmark CSV files generated during the experiments are included in the same repository (\url{https://github.com/maletsden/meshpress}) under \texttt{bench\_*.csv}, archived on Zenodo (\url{https://doi.org/10.5281/zenodo.20942759}); see also Section~\ref{sec:availability}.
