...
All source code, the STRIDE bitstream specification, the test-mesh corpus, raw benchmark CSV files, and the scripts that regenerate every table and figure in this paper are publicly available in the \textbf{MeshPress} repository at \url{https://github.com/maletsden/meshpress}, released under the MIT license and archived with a permanent DOI on Zenodo (DOI: [INSERT ZENODO DOI]).
...
\item \textbf{Data availability.} All test meshes used in the evaluation are publicly available from the Stanford 3D Scanning Repository, the Blender community asset library, and Threedscans.com (CC0), and are documented with provenance and licenses in a dataset README in the MeshPress repository. The raw benchmark CSV files generated during the experiments are included in the same repository (\url{https://github.com/maletsden/meshpress}) under \texttt{bench\_*.csv}, archived on Zenodo (DOI: [INSERT ZENODO DOI]); see also Section~\ref{sec:availability}.
